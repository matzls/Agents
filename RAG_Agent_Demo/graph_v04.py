...
def route_question(state: GraphState) -> str:
    ...
    Returns:
        str: Next node to transition to ('websearch' or 'retrieve')
    """
    try:
        ...
        # Add validation and default behavior
        if source not in ["websearch", "vectorstore"]:
            print(f"Invalid routing response: {source}, defaulting to vectorstore")
            source = "vectorstore"
            
        # Return the exact node name
        return "websearch" if source == "websearch" else "retrieve"
    except Exception as e:
        print(f"Routing error: {e}, defaulting to vectorstore route")
        return "retrieve"
...
def create_structured_output(state: GraphState):
    """Update state with structured output."""
    documents = state.documents
    answer_provided = "yes" if state.final_answer else "no"
...
# Add ALL nodes first, including the router
workflow.add_node("route_question", lambda x: x)  # Add router as a pass-through node
